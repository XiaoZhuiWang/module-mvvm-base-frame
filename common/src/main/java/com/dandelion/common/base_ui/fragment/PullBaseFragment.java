...
/**
 * @author: Lyn Wang
 * @descrption: 支持上下拉加载数据的使用Mvp模式的
 * @date: 2017/01/05.
 */
public abstract class PullBaseFragment<T extends PullBaseVM> extends LoadBaseFragment<T> {
    ...
    private void initRefresh() {
        if (refreshLayout != null) return; //防止Fragment视图被回收，再重建时的重复初始化

        refreshLayout = onAttachRefreshLayout();
        if (refreshLayout != null) {
            //配置刷新
            AppRefreshHeader refreshHeader = new AppRefreshHeader(mContext);
            ...
            //配置加载更多
            refreshLayout.setRefreshFooter(new AppRefreshFooter(mContext));   //设置刷新尾部
